...
from old.Players_old import Player
from old.Lineup_old import Lineup
...
class Projector:
    ...
    def order_players_by_value(self):
        ...
        # self.reduce_players(7, 8, 12, 6, 5)
        #self.reduce_players(5, 7, 9, 5, 5)
    ...
    def lineups_iter(self, r, captain_mode):
        ...
        lineup = Lineup([], 50000, captain_mode=captain_mode)
        for i in indices:
            added = lineup.add_player(self.players[i], captain_mode)
            if not added:
                incur = False
                indices[i] += 1
        ...
        while True:
            ...
            if i < lineup.len_players:
                lineup = Lineup(lineup.players[0:i], captain_mode=captain_mode)
            if incur:
                indices[i] += 1
                for j in range(i+1, r):
                    indices[j] = indices[j-1] + 1
            for k in range(lineup.len_players, r):
                added = lineup.add_player(pool[indices[k]], captain_mode)
                if not added:
                    incur = False
                    indices[k] += 1
